...
{
  "$end", "error", "$undefined", "NUMERO", "MONEDA", "A", "$accept",
  "INICIO", 0
};
#endif
...
#line 49 "monedas.y"
    {
        double tasaOrigen = obtenerTasa((yyvsp[(2) - (4)].texto));
        double tasaDestino = obtenerTasa((yyvsp[(4) - (4)].texto));
        if (tasaOrigen == 0 || tasaDestino == 0) {
            return 0;
        }
        double conversion = (yyvsp[(1) - (4)].numero) * tasaDestino / tasaOrigen;
        printf("CONVERSION EXITOSA\n");
        printf("MONTO: %.2f\n", (yyvsp[(1) - (4)].numero));
        printf("MONEDA_ORIGEN: %s\n", (yyvsp[(2) - (4)].texto));
        printf("MONEDA_DESTINO: %s\n", (yyvsp[(4) - (4)].texto));
        printf("CONVERSION: %.2f %s\n", conversion, (yyvsp[(4) - (4)].texto));
    ;}
    break;
